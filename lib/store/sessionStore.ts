...
export const useSessionStore = create<SessionState>((set, get) => ({
    ...
    advanceDay: () => {
        ...
        const practiceStartIndex =
            state.currentSession?.practiceStartIndex || 0;
    ...
    rewindDay: () => {
        const state = get();
        const prevIndex = state.currentPriceIndex - 1;
        const practiceStartIndex =
            state.currentSession?.practiceStartIndex || 0;

        if (prevIndex >= practiceStartIndex) {
            const visiblePrices = state.stockPrices.slice(0, prevIndex + 1);
            set({
                currentPriceIndex: prevIndex,
                visiblePrices,
            });

            // セッションの現在日を更新（練習期間内での日数）
            if (state.currentSession) {
                const currentDay = Math.max(0, prevIndex - practiceStartIndex);
                const updatedSession = {
                    ...state.currentSession,
                    currentDay,
                };
