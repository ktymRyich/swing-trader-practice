...
export function convertToChartData(stockPrices: StockPrice[]): {
    ...
    for (const price of stockPrices) {
        ...
        // 出来高の色（前日比で決定）
        let color: string = CHART_COLORS.volume.up; // デフォルトは上昇色
...
export function calculateRSI(
    ...
    for (let i = 0; i < changes.length; i++) {
        if (i < period - 1) {
            ...
            const gains =
                slice.filter((c) => c > 0).reduce((sum, val) => sum + val, 0) /
                period;
            const losses =
                Math.abs(
                    slice
...
export function convertToWeeklyData(dailyPrices: StockPrice[]): StockPrice[] {
    ...
    dailyPrices.forEach((price, index) => {
        ...
        const nextDate =
            index < dailyPrices.length - 1
                ? new Date(dailyPrices[index + 1].date)
                : null;
        ...
        const isEndOfWeek =
            ...
            (nextDate &&
                getWeekNumber(currentDate) !== getWeekNumber(nextDate));
        ...
        if (isEndOfWeek) {
            ...
            const weekVolume = currentWeek.reduce(
                (sum, d) => sum + d.volume,
                0,
            );
...
export function convertToMonthlyData(dailyPrices: StockPrice[]): StockPrice[] {
    ...
    dailyPrices.forEach((price, index) => {
        ...
        if (isEndOfMonth) {
            ...
            const monthVolume = currentMonth.reduce(
                (sum, d) => sum + d.volume,
                0,
            );
