...
export async function GET(request: Request) {
    ...
    `,
      ...
      `,
        ...
        console.log(
            `[GET] セッション取得: ${nickname}, 件数: ${sessionsWithData.length}`,
        );
...
export async function POST(request: Request) {
    try {
      ...
      `,
            ...
            console.log(
                `[DELETE] 削除前: ${currentIds.size}件, 削除後: ${newIds.size}件, 削除対象: ${deletedIds.length}件`,
            );

            if (deletedIds.length > 1) {
                console.warn(
                    `[DELETE] 警告: ${deletedIds.length}件のセッションが削除されます！`,
                );
            }
