...
try {
    ...
    const positionsToUpdate = db.prepare(`
    ...
    const transaction = db.transaction(() => {
        for (const position of positionsToUpdate) {
            ...
            if (position.type === 'long') {
                // ロング（買い）: (売却価格 - 購入価格) × 株数
                calculatedProfit = (position.exit_price - position.entry_price) * position.shares;
            } else if (position.type === 'short') {
                // ショート（売り）: (購入価格 - 売却価格) × 株数
                calculatedProfit = (position.entry_price - position.exit_price) * position.shares;
            } else {
                console.warn(`⚠️  不明なタイプ: ${position.type} (ID: ${position.id})`);
                continue;
            }

            // 既存の値をチェック
            const currentProfit = position.profit_loss || 0;
            const isNullOrZero = position.profit_loss === null || position.profit_loss === 0;
            ...
            if (updatedCount <= 10) {
                ...
                console.log(`  建玉: ¥${position.entry_price} × ${position.shares}株`);
                console.log(`  決済: ¥${position.exit_price}`);
                console.log(`  損益: ¥${currentProfit.toLocaleString()} → ¥${calculatedProfit.toLocaleString()}`);
                console.log('');
            }
        }
    });
    ...
    console.log(`利益: ${stats.profit_count}, 損失: ${stats.loss_count}, ゼロ/NULL: ${stats.zero_count}`);
    console.log(`合計損益: ¥${(stats.total_profit || 0).toLocaleString()}`);
    console.log(`平均損益: ¥${(stats.avg_profit || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`);
