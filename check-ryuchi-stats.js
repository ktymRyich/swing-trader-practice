...
try {
    ...
    sessions.forEach((session, idx) => {
        ...
        if (positions.length > 0) {
            const profitsInSession = positions.filter(p => p.profit_loss && p.profit_loss > 0);
            const lossesInSession = positions.filter(p => p.profit_loss && p.profit_loss < 0);
            ...
            console.log(`  セッション合計損益: ¥${totalProfitLoss.toLocaleString()}`);
    ...
    // 利益・損失に分類
    const profits = allClosedPositions.filter(p => p.profit_loss && p.profit_loss > 0);
    const losses = allClosedPositions.filter(p => p.profit_loss && p.profit_loss < 0);
    const zeros = allClosedPositions.filter(p => !p.profit_loss || p.profit_loss === 0);
    ...
    // 平均損益の計算
    const totalProfitLoss = allClosedPositions.reduce((sum, p) => sum + (p.profit_loss || 0), 0);
    const avgProfitLoss = totalProfitLoss / allClosedPositions.length;

    console.log(`\n【平均損益の計算】`);
    console.log(`式: (全ポジションの損益合計) ÷ (ポジション数)`);
    console.log(`  = (${totalProfitLoss.toLocaleString()}) ÷ ${allClosedPositions.length}`);
    console.log(`  = ¥${avgProfitLoss.toLocaleString(undefined, { maximumFractionDigits: 0 })}`);

    // 平均利益の計算
    if (profits.length > 0) {
        const totalProfit = profits.reduce((sum, p) => sum + (p.profit_loss || 0), 0);
        const avgProfit = totalProfit / profits.length;
        console.log(`\n【平均利益の計算】`);
        console.log(`式: (利益ポジションの合計) ÷ (利益ポジション数)`);
        console.log(`  = (${totalProfit.toLocaleString()}) ÷ ${profits.length}`);
        console.log(`  = ¥${avgProfit.toLocaleString(undefined, { maximumFractionDigits: 0 })}`);
    }

    // 平均損失の計算
    if (losses.length > 0) {
        const totalLoss = losses.reduce((sum, p) => sum + (p.profit_loss || 0), 0);
        const avgLoss = totalLoss / losses.length;
        console.log(`\n【平均損失の計算】`);
        console.log(`式: (損失ポジションの合計) ÷ (損失ポジション数)`);
        console.log(`  = (${totalLoss.toLocaleString()}) ÷ ${losses.length}`);
        console.log(`  = ¥${avgLoss.toLocaleString(undefined, { maximumFractionDigits: 0 })}`);
    }

    // 最大利益
    const maxProfit = allClosedPositions.reduce((max, p) => Math.max(max, p.profit_loss || 0), 0);
    console.log(`\n【最大利益】`);
