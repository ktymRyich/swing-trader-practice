...
export default function TradingChart({
    ...
    useEffect(() => {
        ...
        // 高さを計算（親コンテナの高さからボタン部分の約40pxを引く）
        const chartHeight =
    ...
    useEffect(() => {
        ...
        const { candlestickData, volumeData } =
            convertToChartData(displayPrices);
        ...
        if (isNewData && candlestickData.length > 0) {
            ...
            const volumeHistogramData: HistogramData[] = volumeData.map(
                (v) => ({
                    time: v.time,
                    value: v.value,
                    color: v.color,
            ...
            if (chartRef.current) {
                ...
                if (candlestickData.length > 0) {
                    const visibleDataCount = Math.min(
                        50,
        ...
        <div className="w-full h-full flex flex-col p-2">
            ...
            <div className="flex gap-2 mb-2 flex-shrink-0">
                <button
                    ...
                    className={`px-3 py-1 lg:py-1.5 rounded text-xs lg:text-sm font-medium transition ${
                        timeframe === "daily"
